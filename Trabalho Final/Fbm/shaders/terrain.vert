...
float fbm(vec3 point, float H, float lacunarity, float octaves){
    ...
    if (rem > 0)      /* add in ``octaves''  remainder */
...
void main() {
	...
	float h = fbm(vec3(p.x,0.0,p.y),H,lacunarity,octaves) * scale;
	float h2 = fbm(vec3(p2.x,0.0,p2.y),H,lacunarity,octaves) * scale;
	float h3 = fbm(vec3(p3.x,0.0,p3.y),H,lacunarity,octaves) * scale;
	float h4 = fbm(vec3(p4.x,0.0,p4.y),H,lacunarity,octaves) * scale;
	float h5 = fbm(vec3(p5.x,0.0,p5.y),H,lacunarity,octaves) * scale;

	//restricoões na altura
	h = max(-5,h);
	h2 = max(-5,h2);
	h3 = max(-5,h3);
	h4 = max(-5,h4);
